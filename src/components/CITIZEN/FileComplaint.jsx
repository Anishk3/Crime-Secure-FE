...
import animationData from '../../Assets/animations/Animation - fileComplaint.json' // Path to your Lottie animation file
...
const FileComplaint = () => {
    ...
    const [incidentDetail, setIncidentDetails] = useState('')
    ...
    const handleSubmit = (e) => {
        ...
        CitizenService.AddIncident(auth.id, auth.accessToken, obj).then(response => {
            console.log("response received from addincident api", response.data);
            CitizenService.getEmail(auth.email, auth.accessToken).then(response => {
                console.log("email sent", response.data);
            })
            // Show confirmation prompt with logged-in user's name
            alert(`${auth.username} Complaint Filed Successfully!`)
            navigate("/complaints")
        }).catch(error => {
            console.log("error from addincident api", error);
        })
    }
    ...
    return (
        <div>
            ...
            <div className="min-w-screen h-full flex items-center justify-center">
                <div className="text-gray-500 shadow-xl w-full overflow-hidden">
                    <div className="md:flex w-full h-full">
                        {/* Lottie Animation Section with Heading */}
                        <div className="h-full hidden md:block w-1/2 py-10 px-10 flex flex-col justify-center items-center relative top-10">
                            <h2 className="text-3xl font-bold text-indigo-500 mb-4 relative left-56">File a Complaint</h2>
                            <Lottie options={defaultOptions} height={400} width={404} />
                        </div>
                        <div className="w-full md:w-1/2 py-52 px-10">
                            <form onSubmit={handleSubmit}>
                                <div className="mb-5">
                                    <label htmlFor="incidentType" className="block text-gray-700">Incident Type</label>
                                    <select
                                        id="incidentType"
                                        value={incidentType}
                                        onChange={(e) => setIncidentType(e.target.value)}
                                        className="w-full px-4 py-2 border border-gray-300 rounded-md font-sans"
                                    >
                                ...
                                <div className="mb-5">
                                    <label htmlFor="incidentDetail" className="block text-gray-700">Incident Details</label>
                                    <textarea 
                                        id="incidentDetail" 
                                        value={incidentDetail} 
                                        onChange={(e) => setIncidentDetails(e.target.value)} 
                                        className="w-full px-4 py-2 border border-gray-300 rounded-md font-sans"
                                        placeholder="Provide the details of the incident"
                                    />
                                </div>
                                <div className="mb-5 text-center">
                                    <button 
                                        type="submit" 
                                        className="bg-indigo-500 text-white px-6 py-2 rounded-md font-sans"
                                    >
                                        Submit
                                    </button>
                                </div>
...
//         <div>
//             <Navbar/>
//             <div class="min-w-screen h-screen  flex items-center jistify-center ">
...
//                                                 defaultValue={''}
//                                                 placeholder='Enter detailed information about the crime'
//                                             />
//                                         </div>
//                                     </div>
//                                 </div>
//                                 <div class="flex -mx-3">
//                                     <div class="w-full px-3 mb-5">
//                                         <button type='submit' class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">Submit</button>
//                                     </div>
//                                 </div>
//                             </form>
//                         </div>
