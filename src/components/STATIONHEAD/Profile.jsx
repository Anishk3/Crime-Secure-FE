...
import Navbar from './StationheadNavbar';
...
export default function Profile() {
  ...
  const [stationHead, setStationHead] = useState(null);
  ...
  useEffect(() => {
    StationHeadService.getStationHead(auth.id, auth.accessToken)
      .then(response => {
        console.log("response from getStationHead", response.data);
        setStationHead(response.data);
        setUpdatedData({
          stationheadName: response.data.stationheadName,
        ...
        if (response.data.profileImage) {
          StationHeadService.getProfileImage(auth.accessToken,auth.id).then(response=>{
  ...
  const handleSubmit = (event) => {
    ...
    formData.append('stationheadName', updatedData.stationheadName);
    ...
    StationHeadService.updateStationHead(auth.accessToken, formData)
      .then(response => {
  ...
  if (!stationHead) return <div>Loading...</div>;  // Display loading state until data is fetched
              ...
              <div className="sm:col-span-3">
                ...
                <div className="mt-2">
                  <input
                    ...
                    value={updatedData.stationheadName}  // Populate with data
